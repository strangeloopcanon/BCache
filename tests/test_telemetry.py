...
def test_telemetry_logger_append(tmp_path):
    ...
    heat = pd.DataFrame(
        [[0, 0, 1, 1.0]], columns=["layer", "page_id", "decay_hits", "tenant_weight"]
    )
    tiers = pd.DataFrame(
        [[0, 1, 0, 1]], columns=["tier", "free_bytes", "inflight_io", "bandwidth_caps"]
    )
    lats = pd.DataFrame([[0, 1.0]], columns=["layer", "lat_ms"])
    plan = pd.DataFrame(
        [["n0", 0, 1, 0, 0, 0, 1024, 0, 1, 1.0, 0, 1, 256 * 1024, 256 * 1024]],
        columns=[
            "node",
            "tier_src",
            "tier_dst",
