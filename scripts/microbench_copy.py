...
def run_once(page_bytes: int, pages: int, streams: int):
    ...
    for pid in range(pages):
        be.write_page(
            model_id, model_version, layer, pid, page_bytes, secrets.token_bytes(page_bytes)
        )
    ...
    res = adapter.prefetch(
        [req],
        now_ms=now_ms,
        dest_resolver=(lambda info: dst) if dst is not None else None,
...
def main():
    ...
    bytes_copied, dt_ms, ready_events, used_device = run_once(
        args.page_bytes, args.pages, args.streams
    )
