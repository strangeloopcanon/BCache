...
def simulate_plan(
    plan_df: pd.DataFrame, tier_caps_df: pd.DataFrame, window_ms: int = 20
) -> pd.DataFrame:
    ...
    """
    if plan_df.empty:
        return plan_df.assign(
            finish_ms=np.float64(0.0), deadline_rel_ms=np.float64(0.0), on_time=np.int64(1)
        )
    df = plan_df.copy()
...
def simulate_plan_streams(
    ...
    df = df.merge(caps, on="tier_dst", how="left")
    df = df.sort_values(
        by=["node", "tier_dst", "priority"], ascending=[True, True, False]
    ...
    if layer_lat_df is not None and len(layer_lat_df) > 0:
        ...
        cum_deadline = dict(zip(lat["layer"].tolist(), lat["cum_ms"].tolist(), strict=False))

    results = []
    for (node, _tier), grp in df.groupby(["node", "tier_dst"], sort=False):
