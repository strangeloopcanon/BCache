...
def apply_tenant_caps(cand: pd.DataFrame, tenant_caps_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["tenant_cap"] = (
        out["tenant_cap"].astype(float).where(out["tenant_cap"].notna(), 9_223_372_036_854_775_807)
    )
...
def coalesce_intervals(
    ...
    Returns columns:
    node,tier_src,tier_dst,pcluster,layer,run_id,bytes,deadline_ms,fanout,
    urgency_min,start_pid,end_pid,page_bytes.
    """
    ...
    prev_cummax_end = (
        c.assign(_cummax_end=cummax_end).groupby(run_grp)["_cummax_end"].shift(1).fillna(-1)
    )
...
def apply_caps(
    ...
    plan["est_copy_ms"] = (plan["bytes"].astype(float) / np.maximum(denom_cap, 1.0)) * float(
        window_ms
    )
